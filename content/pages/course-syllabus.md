...
> This bound is sharp when \\(X\\) is an arithmetic progression of length \\(N^\alpha\\) with \\(\alpha < 1/2\\). But in this case, \\(\Vert 1_X^{*2} \Vert_{\ell^\infty}\\) is itself large.  Suppose that \\(X \subset [N]\\) with \\(|X| \sim N^{1/2}\\),  and suppose that \\(| 1_X^{_2} |_{L^\infty} \lessapprox 1\\). For most \\(p \in P_{N^{1/2}}\\), can we prove a bound for \\(| (\pi_p 1_X)^{_2}_h |_{L^\infty(\mathbb{Z}_p)}\\) which improves on Equation 1?
